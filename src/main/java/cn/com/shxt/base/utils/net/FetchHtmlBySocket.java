...
/**
 * @author 刘文铭
 * @日期 2012 2012-12-21 下午3:49:46
 * @描述 Java 通过 Socket 的形式抓取网页内容
 * 
 */
public class FetchHtmlBySocket {
	public static void main(String[] args) {
		String html = new FetchHtmlBySocket().htmlContent(new HtmlPage(
		        "blog.csdn.net", "/xia9527"));
	...
	public String htmlContent(HtmlPage hp) {
		...
		try {
			// 建立一个Socket
			socket = new Socket(InetAddress.getByName(hp.getServer()),
			        hp.getPort());
			...
			writer = new BufferedWriter(new OutputStreamWriter(
			        socket.getOutputStream(), hp.getCharset()));
			writer.write(command.toString());
			writer.flush();

			// 接收返回的结果
			reader = new BufferedReader(new InputStreamReader(
			        socket.getInputStream(), hp.getCharset()));
			String line;
